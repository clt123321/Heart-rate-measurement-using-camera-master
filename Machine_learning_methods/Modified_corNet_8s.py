...
# Step 1 load data
# (1.1)data
df_ippg = pd.read_excel('8s_ippg.xlsx')  # good_30s.xlsx,8s_ippg_update_v.xlsx
...
# 按video划分训练集、测试集
s_video = np.unique(df_ippg.loc[:, 'filenames'].values)
...
x_train, y_train = np.array(xx[index_train, :]), np.array(y[index_train])
x_test, y_test = np.array(xx[index_test, :]), np.array(y[index_test])

# (1.5)reshape
nb_time_steps, nb_input_vector = fs * 8, 1
...
model_lstm = model.fit(x_train, y_train,
                       batch_size=64, epochs=200, validation_data=(x_test, y_test),
...
plt.subplot(221)
plt.plot(x_train[0, 0:256], label='data+noise');
