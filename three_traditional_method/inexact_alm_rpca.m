...
%
% lambda - weight on sparse error term in the cost function 稀疏误差项权重
%
...
if nargin < 2   %输入参数的的数量
    lambda = 1 / sqrt(max(m,n));   %源代码的m 修改为 max（m,n）
end
...
A_hat = zeros( m, n);
E_hat = zeros( m, n);
mu = 1.25/norm_two % this one can be tuned  可调节
mu_bar = mu * 1e7;
rho = 1.5          % this one can be tuned  可调节
d_norm = norm(D, 'fro');
